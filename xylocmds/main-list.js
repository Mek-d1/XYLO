...
let handler = async (m, { conn, usedPrefix: _p, args, command }) => {
  try {
    const pluginsDir = path.join(path.dirname(path.dirname(fileURLToPath(import.meta.url))), 'xylocmds')
    
    let pluginFiles;
    try {
      pluginFiles = await fs.readdir(pluginsDir);
      console.log(`Found ${pluginFiles.length} files in xylocmds directory`);
    } catch (e) {
