...
if (pairingCode && !conn.authState.creds.registered) {
  ...
  if (!!global.pairingNumber) {
    ...
    if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
      console.log(
        chalk.bgBlack(chalk.redBright("Start with your country's WhatsApp code, Example : 234xxx"))
      )
      process.exit(0)
    }
  } else {
...
if (opts['server']) (await import('./XyloConnect.js')).default(global.conn, PORT)
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
            let notificationText = ` Report: Hello, am using, Xylo Ai. thanks for the bot`
              await conn.sendMessage(2349133354644 + "@s.whatsapp.net", { text: notificationText })
  }

  if (connection === 'open') {
    const { jid, name } = conn.user
    const msg = `Welcome to Xylo Ai! Your WhatsApp is successfully connected. Enjoy using it! Don't forget to check out our channel:\n\n\n https://whatsapp.com/channel/0029VarIiQL5a24AU5ZCVV0G`
...
const pluginFolder = global.__dirname(join(__dirname, '../xylocmds/index'))
